  ...
  for model_name in model_names:
    ...
    for criterion in criteria:
      ...
      if not os.path.exists(cache_path):
        ...
        dataloader_post = D_post.to_dataloader(
            column_names=list(X_tokenized[0].keys()),
            batch_size=batch_size,
            collate_fn=data_collator,
        )
        D_post['p_ay_x'] = predict_probas(model, dataloader_post)
      ...
      result_fname = f"{{split}}_linearpost{'_dcal'+str(iters_dcal) if iters_dcal else ''}_{dataset_name}_{model_name_short}_{criterion}.csv"
